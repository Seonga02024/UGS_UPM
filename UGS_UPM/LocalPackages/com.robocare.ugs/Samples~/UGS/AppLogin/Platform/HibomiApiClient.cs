...
public sealed class HibomiApiClient
{
    ...
    {
        bool onGetUserData = PlayerPrefs.GetString(PrefKeys.GetUserData).Equals("on");

        if (onGetUserData)
        {
            _ = GetRobotID();
            Debug.Log("신원인식 기능 On 상태, UserData 가져옴");
        }
        else
            Debug.Log("신원인식 기능 Off 상태, UserData 가져오지 않음");
    ...
    public async UniTask<string> GetTokenAndSetCookie()
    {
        ...
        API_URL = PlayerPrefs.GetString(PrefKeys.GameApi, DefaultEndpoints.GameApi);
        ...
        Debug.Log($"{GetType()}::ssoUrl::{ssoUrl}");
        Debug.Log($"{GetType()}::upv::u={Base64Encode(robot)}&p={Base64Encode(password)}&v={Base64Encode(version)}");
        ...
        try
        {
            ...
            {
                Debug.LogError("[API] accessToken 누락됨!");
                return null;
            }
        ...
        {
            Debug.LogError("[API] 토큰 파싱 실패: " + ex.Message);
            return null;
        }
    }
    ...
    private bool SetAuthorizationHeader(HTTPRequest request)
    {
        ...
        {
            Debug.LogWarning("[API] Authorization Token 없음");
            return false;
        }
    ...
    public async UniTask<bool> SendGameScore(List<GameScore> datas)
    {
        ...
        request.RawData = Encoding.UTF8.GetBytes(serializedData);
    ...
    {
        robotAPI_URL = PlayerPrefs.GetString(PrefKeys.RobotApi, DefaultEndpoints.RobotApi);
